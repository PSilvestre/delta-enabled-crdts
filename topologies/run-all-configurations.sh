...
for updates_per_round in updates_per_round_list:
    ...
    for config in configs:
      for fanout in config['fanouts']:
        for delta_or_full_state in ["d", "s"]:
          ...
          cmd = cmd_dir + str(updates_per_round) + "/" + dirname + "/"
          log = log_dir + str(updates_per_round) + "/" + dirname + "/"
          delta_flag = "-" + delta_or_full_state
  
          generate_command = "./generate-replicas-commands.sh {replicas_config} {topology_config} {cmd} {updates}".format(replicas_config = replicas_config, topology_config = config['file'], cmd = cmd, updates = updates_per_round)
          ...
          print "echo \"./analyse.sh " + log + " " + analysis_dir + str(updates_per_round) + "/\" >> analyse-all.sh"
